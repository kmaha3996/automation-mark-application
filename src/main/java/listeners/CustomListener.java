...
import mail.AWSMailer;

public class CustomListener implements IReporter, IExecutionListener {
	private final String titleTxt = "Hodder - MARK Scheduled Automated Test Report";
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		if (System.getProperty("customReportName") != null) {
			outFilename = System.getProperty("customReportName");
		}
		reportFile = new File(outdir, outFilename);
	...
	protected void writeStylesheet() {
		...
		writer.print(".failedodd td,.skipped  {background-color: #FFFF00}");
		writer.print(".failedodd td,.failed  {background-color: #F00}");
		writer.print(".failedeven td,.stripe .attn {background-color: #F00}");
	...
	protected void writeSuiteSummary() {
		...
		for (SuiteResult suiteResult : suiteResults) {
			...
			for (TestResult testResult : suiteResult.getTestResults()) {
				...
				writeTableData(integerFormat.format(skippedTests), (skippedTests > 0 ? "num attn skipped" : "num"));
				writeTableData(integerFormat.format(failedTests), (failedTests > 0 ? "num attn failed" : "num"));
		...
		if (testIndex >= 1) {
			...
			writeTableHeader(integerFormat.format(totalSkippedTests),
					(totalSkippedTests > 0 ? "num attn skipped" : "num"));
			writeTableHeader(integerFormat.format(totalFailedTests),
					(totalFailedTests > 0 ? "num attn failed" : "num"));
	...
	protected void writeScenarioSummary() {
		...
		writer.print("<th>Test Objective</th>");
...
//				int scenariosPerClass = 0;
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					String methodDescription = Utils.escapeHtml(aResult.getMethod().getDescription());
					...
					buffer.append("<td><a href=\"#m").append(scenarioIndex).append("\">")
							.append(methodName + "</a></td>").append("<td rowspan=\"1\">" + methodDescription + "</td>")
							.append("<td rowspan=\"").append(resultsCount).append("\">").append(getDateAsString())
							.append("</td>").append("<td rowspan=\"").append(resultsCount).append("\">")
...
//				writer.print(Utils.escapeHtml(className.substring(className.lastIndexOf("testcases"))));
//				writer.print("</td>");
				writer.print(buffer);
	...
	protected static class TestResult {
		...
		protected static final Comparator<ITestResult> RESULT_COMPARATOR = new Comparator<ITestResult>() {
			public int compare(ITestResult o1, ITestResult o2) {
				...
				if (result == 0) {
					Integer o1Priority = o1.getMethod().getPriority();
					Integer o2Priority = o2.getMethod().getPriority();
	...
	public void onExecutionFinish() {
		String recipients = System.getProperty("recipients", "avanish.bandekar@zeuslearning.com");
		try {
			msgBody = Files.readString(reportFile.toPath());
			AWSMailer.sendMail(recipients, "Automation Smoke Test Report for MARK", msgBody);
		} catch (IOException e) {
			e.printStackTrace();
